...
<html lang="en">
  ...
  <body>
    <div class="container">
      ...
      <div class="modeldescr">
        <p>Model path: <code>{{ model_path }}</code></p>
        <p>Model loaded: <strong>{{ "Yes" if model_loaded else "No" }}</strong></p>
      </div>
      <form action="/predict-form" method="post">
        <label for="message"><strong>Enter text to classify:</strong></label><br>
        <textarea id="message" name="message" required>{{ request.form.get("message", "") }}</textarea>
